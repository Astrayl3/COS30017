...
class MainActivity : ComponentActivity() {
    ...
    // Variables to track the score, the current wall hold, and if the climber has fallen
    private var score_display = 0
    private var currentWallHold = 0
    private var hasFallen = false

    // UI elements for displaying the score and handling user interactions
    ...
    // Set up the UI and restore the state if available
    ...
    // Get the color of the displayed score based on the current wall hold
    ...
    // Update the color of the displayed score based on the current wall hold
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        outState.putInt("score_display", score_display)
